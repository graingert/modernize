...
def main(argv=sys.argv):
    ...
    with tmp_requirements(hashed_reqs) if hashed_reqs else contextlib.nullcontext(()) as requirements:
        # pip does not process `--hash` args unless in a requirements.txt
        subprocess.run(
            [sys.executable, "-m", "pip", "install", "--require-hashes", *requirements, *not_hashed_reqs],
            check=True,
        )
